...
export default function PgAdminSendLettersPage() {
  ...
  useEffect(() => {
    if (!isAuthenticated || !["pgadmin", "pgdean"].includes(user?.role || "")) {
  ...
  const loadData = async () => {
    ...
    try {
      const [facultyData, statusData] = await Promise.all([
        ...
        ApiClient.getPgLetterStatusSummary(),
      ]);
  ...
  const loadDepartmentApplicants = async (department: string) => {
    try {
      const response = await ApiClient.getPgDepartmentApplicants(department);
  ...
  const handleSendLetters = async () => {
    if (!selectedDepartment || selectedApplicants.size === 0) {
      setError("Please select a programme and applicants");
      return;
    }
    setSending(true);
    setError(null);
    ...
    try {
      const result = await ApiClient.sendPgDepartmentLetters(
        selectedDepartment,
  ...
  const handleResend = async (applicantId: number | string) => {
    try {
      await ApiClient.resendPgLetter(applicantId, admissionDate);
  ...
  if (!isAuthenticated || !["pgadmin", "pgdean"].includes(user?.role || "")) return null;
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-muted-foreground">
            Manage and send postgraduate admission letters by programme
          </p>
        </div>
            ...
            {loading ? (
              ...
              <Card>
                <CardContent className="pt-12 text-center">
                  ...
                  <p className="text-muted-foreground">
                    All postgraduate applicants have received their letters!
                  </p>
                </CardContent>
              </Card>
            ) : (
                      ...
                      <>
                        ...
                        <div className="border-t border-border pt-4">
                          <div className="space-y-3">
                            <div>
                              <Label htmlFor="pg-admission-date">
                                Admission Date
                              </Label>
                              <Input
                                id="pg-admission-date"
                                type="date"
                                value={admissionDate}
