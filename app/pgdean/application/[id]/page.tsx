  ...
  return (
    <div className="space-y-6 bg-white border border-gray-200 p-6 rounded-xl">
      ...
      <div className="flex flex-col md:flex-row items-start gap-6 border-b border-gray-100 pb-6">
        ...
        <div className="space-y-1.5 flex-1 pt-1">
          ...
          <div className="flex flex-wrap gap-4 text-sm text-slate-500">
            <p><strong className="text-slate-700">Email:</strong> {form?.email || applicant?.email}</p>
            <p><strong className="text-slate-700">Phone:</strong> {form?.phone_number || applicant?.phone_number}</p>
            {form?.secondary_phone_number && (
              <p><strong className="text-slate-700">Alt Phone:</strong> {form.secondary_phone_number}</p>
            )}
          </div>
          <div className="pt-1">
            <Badge className="bg-slate-100 text-slate-600 border-slate-200 font-medium rounded-md px-2.5 py-1 text-xs">
              {form?.degree_name && form?.proposed_course_name
                ...
                : (form?.proposed_course_name || applicant?.program_name || "N/A")}
            </Badge>
          </div>
        </div>
      </div>
      ...
      <div className="space-y-3">
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-gray-100 pb-2">Personal Details</h3>
      ...
      {/* Academic History */}
      <div className="space-y-3">
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-gray-100 pb-2">Academic History</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          ...
          <InfoCard label="Course of Study" value={form?.previous_course} colSpan="md:col-span-2" />
      ...
      {/* Proposed Study */}
      <div className="space-y-3">
        <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-gray-100 pb-2">Proposed Study</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
          <InfoCard
            label="Degree in View"
            value={form?.degree_name ? `${form.degree_name}${form.degree_code ? ` (${form.degree_code})` : ""}` : undefined}
          />
          <InfoCard label="Proposed Course" value={form?.proposed_course_name} colSpan="md:col-span-2" />
          <InfoCard label="Faculty / Institute" value={form?.proposed_faculty_name} colSpan="md:col-span-2" />
          ...
          {form?.proposed_research_title && (
            <InfoCard label="Research Title" value={form.proposed_research_title} colSpan="md:col-span-3" />
          )}
        </div>
      </div>
      ...
      {(form?.referee_name1 || form?.referee_name2 || form?.referee_name3) && (
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-gray-100 pb-2">Referees</h3>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            {[
            ...
            ].filter((r) => r.name).map((ref, idx) => (
              <div key={idx} className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-2">
                <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">{ref.label}</p>
                <div className="flex justify-between text-sm border-b border-gray-100 pb-2">
                  <span className="text-slate-400 font-medium shrink-0">Name</span>
                  <span className="font-semibold text-slate-700 text-right max-w-[180px] leading-snug">{ref.name}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span className="text-slate-400 font-medium shrink-0">Address</span>
                  <span className="font-medium text-slate-600 text-right max-w-[180px] leading-snug">{ref.address || "N/A"}</span>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-gray-100 pb-2">Sponsor</h3>
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-3">
            {[
              { label: "Name", value: form?.sponsor_name },
              { label: "Address", value: form?.sponsor_address },
            ].map((item, idx) => (
              <div key={idx} className="flex justify-between text-sm border-b border-gray-100 pb-2 last:border-0 last:pb-0">
                <span className="text-slate-400 font-medium">{item.label}</span>
                <span className="font-semibold text-slate-700 text-right max-w-[180px]">{item.value || "N/A"}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="space-y-3">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider border-b border-gray-100 pb-2">Next of Kin</h3>
          <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 space-y-3">
            {[
      ...
      {form?.document_signature && (
        <div className="space-y-2 border-t border-gray-100 pt-5">
          <h3 className="text-xs font-bold text-slate-400 uppercase tracking-wider">Applicant's Signature</h3>
...
function DocumentsTab({ documents, applicationId }: { documents: any[]; applicationId: string }) {
  const handleDownload = async (doc: any) => {
    try {
      const token = localStorage.getItem("auth_token");
      const baseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/e-portal/api";
      const res = await fetch(`${baseUrl}/applicant/download-document/${doc.id || doc.document_id}`, {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
      <div className="p-4 border-b border-gray-100">
        ...
        <p className="text-slate-400 text-xs mt-0.5">{documents.length} document(s)</p>
      </div>
      <div className="p-4">
        {documents.length > 0 ? (
            ...
            {documents.map((doc) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  <p className="font-semibold text-slate-700 text-sm truncate">{doc.original_filename}</p>
    ...
    <div className="space-y-4">
      ...
      {evaluation && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-start gap-3">
          ...
          <div>
            <p className="text-emerald-700 font-semibold text-sm">Section B previously completed</p>
            <p className="text-emerald-600/70 text-xs mt-0.5">
              By {evaluation.dean_name || "Dean"} on{" "}
              {evaluation.updated_at ? new Date(evaluation.updated_at).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" }) : "—"}
            </p>
          </div>
        </div>
      )}
      ...
      {saveSuccess && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center gap-2">
          ...
          <p className="text-emerald-700 font-semibold text-sm">Section B saved. Application sent to Admissions Officer.</p>
        </div>
      )}
      ...
      <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
        <div className="px-5 pt-5 pb-4 border-b border-gray-100">
          <h3 className="text-sm font-bold text-slate-800">SECTION B: EVALUATION AND RECOMMENDATION</h3>
          <p className="text-slate-400 text-xs mt-0.5 italic">(To be completed by the Dean)</p>
        </div>
        <div className="p-5 space-y-6">
          ...
          <div className="space-y-2.5">
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
              <span className="w-6 h-6 rounded-md bg-slate-100 text-slate-500 flex items-center justify-center text-xs font-bold shrink-0">1</span>
              Transcript Received
            </label>
            ...
            <div>
              <label className="text-xs font-semibold text-slate-400 uppercase tracking-wider block mb-1">Comment</label>
              <input
                type="text"
          ...
          <div className="space-y-2.5">
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
              <span className="w-6 h-6 rounded-md bg-slate-100 text-slate-500 flex items-center justify-center text-xs font-bold shrink-0">2</span>
              Number of Reference Letters Received
            </label>
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={() => setRefLettersCount(Math.max(0, refLettersCount - 1))}
              ...
              <input
                ...
                onChange={(e) => setRefLettersCount(parseInt(e.target.value) || 0)}
              ...
              <button
                type="button"
                onClick={() => setRefLettersCount(Math.min(10, refLettersCount + 1))}
                className="w-9 h-9 rounded-lg bg-gray-100 border border-gray-200 text-slate-600 hover:bg-gray-200 font-bold text-base transition-all"
              >
                +
              </button>
          ...
          <div className="space-y-2.5">
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
              <span className="w-6 h-6 rounded-md bg-slate-100 text-slate-500 flex items-center justify-center text-xs font-bold shrink-0">3</span>
              Recommendation
            </label>
            <textarea
              rows={4}
          ...
          <div className="space-y-2.5">
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
              <span className="w-6 h-6 rounded-md bg-slate-100 text-slate-500 flex items-center justify-center text-xs font-bold shrink-0">4</span>
              Name of Supervisor
            </label>
            <input
              type="text"
          ...
          <div className="space-y-2.5">
            <label className="flex items-center gap-2 text-sm font-semibold text-slate-700">
              <span className="w-6 h-6 rounded-md bg-slate-100 text-slate-500 flex items-center justify-center text-xs font-bold shrink-0">5</span>
            ...
            <div className="bg-gray-50 border border-gray-200 rounded-lg p-4 flex flex-col sm:flex-row sm:items-center gap-4">
              {evaluation?.dean_name ? (
                <div className="space-y-0.5">
                  <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Signed by</p>
                  <p className="font-semibold text-slate-700 text-sm">{evaluation.dean_name}</p>
                </div>
              ) : (
                <div className="space-y-0.5">
                  <p className="text-xs text-slate-400 font-semibold uppercase tracking-wider">Will be signed as</p>
                  <p className="font-semibold text-slate-700 text-sm">Dean of Postgraduate School</p>
                </div>
              )}
              <div className="sm:ml-auto space-y-0.5">
                ...
                <p className="font-semibold text-slate-700 text-sm">
                  {evaluation?.updated_at
                    ? new Date(evaluation.updated_at).toLocaleDateString("en-GB", { day: "numeric", month: "long", year: "numeric" })
...
export default function PgApplicationDetailPage() {
  ...
  const handlePrint = async () => {
    ...
    try {
      const token = localStorage.getItem("auth_token");
      const baseUrl = process.env.NEXT_PUBLIC_API_URL || "http://localhost:5000/e-portal/api";
      const res = await fetch(`${baseUrl}/pgdean/print-application/${applicationId}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gray-50">
        <div className="text-center">
          ...
          <p className="text-slate-700 font-bold text-base mb-1">Application Not Found</p>
          <p className="text-slate-400 text-sm mb-4">{error}</p>
          <Link href="/pgdean/applications" className="text-slate-500 hover:text-slate-700 font-semibold transition-colors text-sm">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 bg-white border border-gray-200 rounded-xl p-5 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-1.5">
            ...
            <div className="flex flex-wrap gap-4 text-sm text-slate-500">
              <span>Form No: <strong className="text-slate-700 font-mono">{applicant?.form_no || "N/A"}</strong></span>
              <span>Session: <strong className="text-slate-700">{applicant?.session || "N/A"}</strong></span>
              <span>Programme: <strong className="text-slate-700">{form?.proposed_course_name || applicant?.program_name || "N/A"}</strong></span>
            </div>
          </div>
        </div>
        ...
        <Tabs defaultValue="info">
          <TabsList className="bg-white border border-gray-200 rounded-lg p-1 mb-5 gap-1">
            {[
              ...
              { value: "section-b", label: "Section B Eval.", icon: ClipboardCheck },
          ...
          <TabsContent value="info">
            <ApplicantInfoTab form={form} passportUrl={passportUrl} applicant={applicant} />
          </TabsContent>

          <TabsContent value="documents">
            <DocumentsTab documents={documents} applicationId={applicationId} />
          </TabsContent>
