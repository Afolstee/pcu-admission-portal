...
export default function PgApplicationsPage() {
  ...
  const handleDownload = async (app: PgApplication, e: React.MouseEvent) => {
    ...
    try {
      ...
      const res = await fetch(`${baseUrl}/pgdean/print-application/${app.id}`, {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-6 flex flex-col md:flex-row md:items-center justify-between gap-4">
          ...
          <Select value={status} onValueChange={setStatus}>
            <SelectTrigger className="w-48 h-10 bg-white border-gray-200 text-slate-700 font-medium rounded-lg">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="bg-white border-gray-200 rounded-lg shadow-lg">
              <SelectItem value="submitted" className="text-slate-700 font-medium cursor-pointer">New Submissions</SelectItem>
              <SelectItem value="screening" className="text-slate-700 font-medium cursor-pointer">Under Review</SelectItem>
        ...
        {loading ? (
          ...
          <div className="space-y-2">
            {applications.map((app) => (
              <Link
                key={app.id}
                href={`/pgdean/application/${app.id}`}
                className="block"
              >
                      ...
                      <div className="flex-1 min-w-0 space-y-2">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {app.has_evaluation ? (
                            ...
                            <span className="flex items-center gap-1 text-[10px] font-semibold px-2 py-0.5 rounded-md bg-amber-50 text-amber-700 border border-amber-200">
