...
export default function PgDeanDashboard() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await ApiClient.getPgDeanDashboard(10);
        setStats(res.statistics);
        setActivity(res.recent_activity || []);
      } catch (err) {
        console.error("PG Dean dashboard error:", err);
      } finally {
        setLoading(false);
      }
    })();
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
        ...
        {/* Header */}
        <div className="mb-8">
          <h1 className="text-2xl font-bold text-slate-800">Good day, Admin</h1>
          <p className="text-slate-500 text-sm mt-0.5">Postgraduate School</p>
        </div>

        {/* Quick actions */}
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 mb-8">
          <Link
            href="/pgdean/applications?status=submitted"
          ...
          <Link
            href="/pgdean/applications?status=screening"
            className="group flex items-center gap-4 bg-white hover:bg-slate-50 border border-gray-200 rounded-xl p-4 transition-colors"
          >
