...
const statusColors: Record<string, string> = {
  ...
  incomplete: "bg-slate-50 text-slate-600 border border-slate-200/60 shadow-sm shadow-slate-500/5",
  payment_pending: "bg-amber-50 text-amber-700 border border-amber-200/60 shadow-sm shadow-amber-500/5",
};
...
function ApplicationsContent() {
  ...
  const loadApplications = async () => {
    setLoading(true);
    try {
      const response = await ApiClient.getPtApplications(status, page, 10);
  ...
  const handleStatusChange = (nextStatus: string) => {
    ...
    setPage(1);
    router.replace(`/ptadmin/applications?status=${nextStatus}&page=1`, {
      scroll: false,
    });
  };
  ...
  const handlePageChange = (nextPage: number) => {
    ...
    setPage(nextPage);
    router.replace(`/ptadmin/applications?status=${status}&page=${nextPage}`, {
      scroll: false,
    });
  };
  ...
  return (
    <div className="min-h-screen bg-[#f3eee6]">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-slate-500 font-medium">
              Review and manage part-time applicant submissions
            </p>
          </div>
          <Select value={status} onValueChange={handleStatusChange}>
            <SelectTrigger className="w-52 h-11 bg-white border-[#e8dfd2] shadow-sm rounded-xl font-bold">
              <SelectValue />
            </SelectTrigger>
            <SelectContent className="bg-white/95 backdrop-blur-md rounded-xl border-slate-100 shadow-xl">
              ...
              <SelectItem value="incomplete" className="font-semibold text-slate-600 focus:text-[#c99b45] focus:bg-[#fdf8f0] cursor-pointer">Docs Incomplete</SelectItem>
              <SelectItem value="payment_pending" className="font-semibold text-slate-600 focus:text-[#c99b45] focus:bg-[#fdf8f0] cursor-pointer">Awaiting Payment</SelectItem>
            </SelectContent>
          </Select>
        </div>
        ...
        {/* Applications List */}
        {loading ? (
          <div className="text-center py-20 bg-white border border-[#e8dfd2] rounded-3xl shadow-sm">
          ...
          <Card className="border-dashed border-2 border-[#e8dfd2] shadow-inner bg-white/50 rounded-3xl overflow-hidden">
            <CardContent className="py-20 text-center max-w-sm mx-auto space-y-4">
              <div className="w-16 h-16 bg-white border border-[#e8dfd2] shadow-md rounded-2xl flex items-center justify-center mx-auto text-slate-300">
                <FileText className="w-8 h-8" />
              </div>
              <div className="space-y-1">
                <p className="text-slate-800 font-bold text-lg">No Applications Found</p>
                <p className="text-slate-400 text-sm leading-relaxed font-medium">
                  No part-time applications match the{" "}
          ...
          <div className="space-y-4">
            {applications.map((app) => (
              <Link
                key={app.id}
                href={`/ptadmin/application/${app.id}?status=${status}`}
                className="block"
              >
                      ...
                      <div className="min-w-0 flex-1 space-y-5">
                        ...
                        <div className="grid grid-cols-1 gap-4 text-sm sm:grid-cols-2 xl:grid-cols-4">
                          <div className="rounded-xl border border-[#eee5d8] bg-[#fbfaf7] p-3">
                            ...
                            <p className="mt-1 font-bold text-slate-800 font-mono text-sm break-words">
                              {app.form_no || "N/A"}
                            </p>
                          </div>
                          <div className="rounded-xl border border-[#eee5d8] bg-[#fbfaf7] p-3">
                            <span className="text-xs text-slate-500 font-bold">Email Address</span>
                            <p className="mt-1 font-bold text-slate-800 text-sm break-words">
                              {app.email}
                            </p>
                          </div>
                          <div className="rounded-xl border border-[#eee5d8] bg-[#fbfaf7] p-3 sm:col-span-2 xl:col-span-1">
                            <span className="text-xs text-slate-500 font-bold">Programme</span>
                            <p className="mt-1 font-black text-slate-900 text-sm break-words">
                              {app.program_name}
                            </p>
                          </div>
                          <div className="rounded-xl border border-[#eee5d8] bg-[#fbfaf7] p-3">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="flex items-center gap-1">
                    {buildPageNums().map((item, idx) =>
                      item === "..." ? (
                        <span key={`ellipsis-${idx}`} className="px-2 text-slate-400 font-bold text-sm">
                          …
                        </span>
                      ) : (
                        <button
