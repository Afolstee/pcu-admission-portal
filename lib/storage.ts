...
export async function getSessionResults(sessionId: string | number): Promise<StudentSessionTranscript[]> {
  const res = await fetch(`/api/results/session/${encodeURIComponent(String(sessionId))}`)
